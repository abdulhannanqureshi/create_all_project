...
if(isset($_GET["currency-unit"]) && in_array($_GET["currency-unit"], get_currency_unit_list()) && ((isset($_GET["min-price"]) && !empty($_GET['min-price'])) || (isset($_GET["max-price"]) && !empty($_GET['max-price']))) ){
 ...
 $last_C_unit = $_COOKIE["site-currency-unit"];
  ...
  if($last_C_unit == get_base_currency_unit()){
    ...
    if(isset($_GET["min-price"]) && !empty($_GET['min-price'])){
      $_GET["min-price"] = round(($_GET["min-price"] * $new_C_rate/$last_C_rate ) , 2);
    }
    if(isset($_GET["max-price"]) && !empty($_GET['max-price'])){
      $_GET["max-price"] = round(($_GET["max-price"] * $new_C_rate/$last_C_rate )  , 2);
    }
  }
...
$sort_form_action = get_site_url().'/property/';
    ...
    <div class="inner-page-wrap">
        <section class="inner-banner">
    		...
    		<div class="banner-content wow bounceInUp">
    			...
    			<h1 class="text-center m-0"><?php echo "Property Listing"; ?></h1>
      ...
      ?>
      <section class="property-search-wrapper" id="inline-search-form">
        <button class="filter-result btn btn-primary"> Filter Results</button>
        <div class="search-addon-section property-search">
        ...
        <nav aria-label="Page navigation example">
            ...
            if($total_page > 1){
                ...
                if($current_page != 1){
                    ...
                    <a class="page-link"  aria-label="Previous" href="'.$pagi_link.''.($current_page - 1).'"><span aria-hidden="true">&laquo;</span></a></li>';
                }
                ...
                for($i=1; $i<= $total_page; $i++){
                    if($current_page == $i){
                        ...
                        echo '<li class="page-item"><a class="page-link" href="'.$pagi_link.''.$i.'">'.$i.'</a></li>';
                    }
                }

                if($current_page != $total_page){
                    echo '<li class="page-item">
                    <a class="page-link" aria-label="Next" href="'.$pagi_link.''.($current_page + 1).'"><span aria-hidden="true">&raquo;</span></a></li> ';
                }
            }
            ?>
            </ul>
...
<script>
    ...
    // Get the navbar
    var navbar = document.getElementById("inline-search-form"); 
...
<script>
    jQuery(document).ready(function(){
        jQuery('.filter-result').click(function(){
            jQuery('.property-search').slideToggle();
        });
    });
</script>
<?php get_footer(); ?>